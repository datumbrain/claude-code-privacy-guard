...
function formatFindingsList(result: ScanResult): string {
 ...
 */
export function buildHookResponse(result: ScanResult, mode: PromptGuardMode): HookResponse | null {
  ...
  if (mode === 'warn') {
    return {
      systemMessage:
        `🛡️ Privacy Guard: sensitive data detected (warn mode - prompt allowed through)\n\n` +
